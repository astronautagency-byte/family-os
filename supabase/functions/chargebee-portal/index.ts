...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: sub } = await admin.from("account_subscriptions").select("chargebee_customer_id").eq("household_id", membership.household_id).maybeSingle();
    if (!sub?.chargebee_customer_id) return reply({ error: "No Chargebee account yet." }, 404);
    const form = new URLSearchParams({ "customer[id]": sub.chargebee_customer_id, redirect_url: `${Deno.env.get("FRONTEND_URL") || "https://fam-os.app"}/#settings` });
