...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!features.length) return reply({ error: "Choose at least one feature." }, 400);
    const baseItem = Deno.env.get("CHARGEBEE_ITEM_FREE_BASE");
    if (!baseItem) throw new Error("Chargebee free base item price is missing.");
    ...
    features.forEach((feature, index) => {
      form.set(`subscription_items[item_price_id][${index}]`, featureItemPrice(feature));
      form.set(`subscription_items[quantity][${index}]`, "1");
    });
    let path = "/hosted_pages/checkout_new_for_items";
    if (current?.chargebee_subscription_id) {
      path = "/hosted_pages/checkout_existing_for_items";
      form.set("subscription[id]", current.chargebee_subscription_id);
      form.set("replace_items_list", "false");
    } else {
      form.set(`subscription_items[item_price_id][${features.length}]`, baseItem);
      form.set(`subscription_items[quantity][${features.length}]`, "1");
    ...
    const result = await chargebeeRequest(path, form);
