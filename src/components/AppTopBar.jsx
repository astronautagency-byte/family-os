import { Bell, CalendarDays, CheckSquare, Home, Moon, Settings2, ShoppingCart, Sun, Tablet, X } from "lucide-react";
import { useMemo, useState } from "react";
import { useAuth } from "../context/AuthContext";
import { useFamily } from "../context/FamilyContext";
import { todayISO } from "../lib/dates";

export default function AppTopBar({ onOpenSettings, onNavigate, darkMode, onToggleDarkMode, tabletMode, onToggleTabletMode }) {
  ...
  return <header className="app-topbar">
    ...
    <div className="topbar-actions">
      <button className="m3-icon-button" aria-label={`${unread.length} unread notifications`} aria-expanded={open} onClick={()=>setOpen(v=>!v)}><Bell/>{unread.length>0&&<i>{unread.length}</i>}</button>
