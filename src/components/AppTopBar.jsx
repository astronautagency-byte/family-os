import { Bell, CalendarDays, CheckSquare, Settings2, ShoppingCart, X } from "lucide-react";
...
export default function AppTopBar({ onOpenSettings, onNavigate }) {
  ...
  return <header className="app-topbar">
    ...
    <div className="topbar-actions"><button aria-label={`${unread.length} unread notifications`} aria-expanded={open} onClick={()=>setOpen(v=>!v)}><Bell/>{unread.length>0&&<i>{unread.length}</i>}</button><button aria-label="Settings" onClick={onOpenSettings}><Settings2/></button></div>
