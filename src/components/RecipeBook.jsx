...
import {BookOpen,Link2 as Link,ImagePlus,Search,ChefHat,CalendarPlus,Pencil,ScanLine as ScanText,Bookmark,ListChecks} from "./icons";
 ...
 return <Modal open title="Recipe Book" onClose={()=>!busy&&onClose()}><div className="recipe-book">
  ...
  {loaded&&<fieldset disabled={busy}>
   ...
   <section><h3><BookOpen size={20}/> Your recipes</h3><label><span className="recipe-book-label"><Search size={16}/> Find a recipe</span><input type="search" placeholder="Search by name or ingredient" value={search} onChange={e=>setSearch(e.target.value)}/></label>
    ...
    <div className="recipe-book-grid">{visible.map(row=><article key={row.id}><span className="recipe-book-category"><BookOpen size={16}/> FAMILY RECIPE</span><h4>{row.title}</h4><p className="recipe-book-meta"><ListChecks size={15}/>{row.recipe.ingredients?.length || 0} ingredients{row.recipe.readyInMinutes?` · ${row.recipe.readyInMinutes} min`:''}</p><div className="recipe-book-card-actions"><SecondaryButton onClick={()=>{onClose();onCook({...row.recipe,id:row.id});}}><ChefHat size={17}/> Open recipe</SecondaryButton><SecondaryButton onClick={()=>setPlan(row)}><CalendarPlus size={17}/> Plan meal</SecondaryButton></div>{row.recipe.sourcePhotos?.length>0&&<details><summary>Original photos</summary><div className="recipe-book-photos">{row.recipe.sourcePhotos.map((p,i)=><img key={i} src={p} alt={`Source page ${i+1}`}/>)}</div></details>}</article>)}</div>
   </section>
  </fieldset>}
 </div></Modal>;
}
