...
import { Badge, CircularProgress, DateField, Modal, PrimaryButton, TextField } from "../components/ui";
...
export default function KitchenWatch({ goTo }) {
  ...
  return <PullToRefresh onRefresh={refreshData}><div className="pb-24 kitchen-watch-page">
    ...
    <section className="kitchen-inventory-card" aria-labelledby="kitchen-watch-items-title">
      ...
      {visibleItems.length ? <div className="inventory-item-grid">{visibleItems.map((item) => { const expiry = inventoryExpiryStatus(item); const progress = inventoryExpiryProgress(item); const onList = isIngredientOnList(item.name, groceries); const expiryDate = item.expiresOn ? new Date(`${item.expiresOn}T12:00`) : null; const expiryLabel = expiryDate ? expiryDate.toLocaleDateString(undefined, { month: "short", day: "numeric" }) : "Set date"; const ringColor = expiry?.state === "expired" ? "var(--color-warn)" : expiry?.state === "soon" ? "var(--color-meals)" : "var(--color-shopping)"; return <article key={item.id} className={expiry ? `is-${expiry.state}` : ""}><div className="inventory-item-copy">{item.imageUrl ? <img src={item.imageUrl} alt=""/> : <FreshItemIcon category={item.category}/>}<div><span>{item.category}</span><strong>{item.name}</strong>{item.brand && <small>{item.brand}</small>}{expiry && <Badge tone={expiry.state === "expired" ? "warn" : "good"}>{expiry.label}</Badge>}</div></div><div className="inventory-expiry-control">{progress && <CircularProgress value={progress.remainingPercent} color={ringColor} label={`${progress.remainingPercent}% freshness left`}><strong>{expiryLabel}</strong></CircularProgress>}<DateField compact label="Use by" value={item.expiresOn} onChange={(expiresOn) => updateItem(item.id, { expiresOn })}/></div><div className="inventory-item-actions"><div className="inventory-quantity"><button type="button" onClick={() => updateItem(item.id, { quantity: Math.max(1, Number(item.quantity || 1) - 1) })} disabled={Number(item.quantity) <= 1}><Minus size={12}/></button><strong>{item.quantity}{item.unit ? ` ${item.unit}` : ""}</strong><button type="button" onClick={() => updateItem(item.id, { quantity: Number(item.quantity || 1) + 1 })}><Plus size={12}/></button></div>{expiry?.state === "expired" && <button type="button" disabled={onList} onClick={() => replaceItem(item)}><RotateCcw size={13}/>{onList ? "On list" : "Replace"}</button>}<button type="button" onClick={() => removeItem(item.id)}><Check size={13}/>Used up</button></div></article>; })}</div> : <div className="inventory-empty"><span><Store size={20}/></span><strong>{query || status !== "all" ? "No matching fresh items" : `No fresh food in the ${location}`}</strong><p>{query || status !== "all" ? "Clear the filters or search another item." : "Add a fresh item, or check off perishables in Shopping and review them here."}</p>{query || status !== "all" ? <button type="button" onClick={() => { setQuery(""); setStatus("all"); }}>Clear filters</button> : null}</div>}
    </section>
