...
import { Alert, Avatar, Checkbox, DateField, Modal, PrimaryButton, ProgressBar, TextField } from "../components/ui";
...
export default function Tasks(){
 const {members,memberById,tasks,taskLists=[],addTaskList,addTask,toggleTask,updateTask,removeTask,clearTasks,refreshData}=useFamily();
 const [clearing,setClearing]=useState(false);
 const [editingId,setEditingId]=useState(null);
 const [draft,setDraft]=useState({title:"",assigneeId:members[0]?.id||"",due:todayISO(),taskType:"home",listId:null});
 ...
 const openEdit=(task)=>{setEditingId(task.id);setDraft({title:task.title,assigneeId:task.assigneeId||"",due:task.due||todayISO(),taskType:task.taskType||"home",listId:task.listId||null});setShowEditPanel(true);};
 ...
 <form className="task-inline-form" onSubmit={submitInline}>
  ...
  <Modal open={showEditPanel} onClose={()=>setShowEditPanel(false)} title={editingId?"Edit task":"Add task"}><TextField label="Task" value={draft.title} onChange={e=>setDraft({...draft,title:e.target.value})}/><DateField label="Due date" value={draft.due} onChange={due=>setDraft({...draft,due})}/><p className="task-assignee-label">List</p><div className="task-category-picker">{Object.entries(GROUPS).map(([key,meta])=><button key={key} className={!draft.listId&&draft.taskType===key?"selected":""} style={{"--task-color":meta.color}} onClick={()=>setDraft({...draft,taskType:key,listId:null})}><i/>{meta.label}</button>)}{taskLists.map(list=><button key={list.id} className={draft.listId===list.id?"selected":""} style={{"--task-color":list.color}} onClick={()=>setDraft({...draft,listId:list.id})}><i/>{list.name}</button>)}</div><p className="task-assignee-label">Assign to</p><div className="task-assignee-picker">{members.map(member=><button key={member.id} className={draft.assigneeId===member.id?"selected":""} onClick={()=>setDraft({...draft,assigneeId:member.id})}><Avatar member={member}/><span>{member.name}</span></button>)}</div><PrimaryButton onClick={save}>Save changes</PrimaryButton></Modal>
