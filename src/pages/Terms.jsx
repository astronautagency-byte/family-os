...
export default function Terms({ signedIn = false }) {
  return <div className="legal-page">
    ...
    <main className="legal-content">
      ...
      <section><h2>Subscription plans and trial</h2><p>FamOS may offer a free {PRICING_PLAN.trial.days}-day trial with full feature access. Unless cancelled before the trial ends, the selected subscription begins automatically at the then-current price shown at checkout. The base family plan starts at {formatMoney(PRICING_PLAN.basePlan.price.monthly)}/month or {formatMoney(PRICING_PLAN.basePlan.price.yearly)}/year and includes up to {PRICING_PLAN.basePlan.membersIncluded} family members. Additional members are charged at {formatMoney(PRICING_PLAN.basePlan.additionalMemberPrice.monthly)}/month per member unless a different price is shown at checkout.</p></section>
      <section><h2>Yearly billing</h2><p>If you choose yearly billing, the base plan is charged upfront after the free trial and renews automatically each year unless cancelled before the renewal date. Additional members and monthly add-ons may be charged monthly or annualized at checkout depending on the billing option shown.</p></section>
      <section><h2>Add-ons</h2><p>Optional add-ons may be billed in addition to the base plan. Fam AI is currently priced at {formatMoney(PRICING_PLAN.addOns[0].price.monthly)}/month and includes {PRICING_PLAN.addOns[0].queryCapPerMonth} assistant requests per month. Fam AI is preselected during the trial. Rewards & points are included in the base plan. Add-ons may be enabled, disabled, or repriced as described at checkout or in your account settings.</p></section>
