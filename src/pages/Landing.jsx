...
const features = [
  { label: "Calendar", title: "Every calendar. One family view.", previewHeadline: "Five calendars, with privacy built in.", copy: "Connect Google, Apple, Outlook, school, sports, or iCal feeds; choose private or shared; and use two-way Google sync.", icon: CalendarDays, art: "calendar", tone: "lilac" },
  { label: "Meals", title: "Recipes that lead to dinner.", previewHeadline: "Pictures, instructions, then hands-free Cook Mode.", copy: "Discover Spoonacular recipes, plan every meal type, add missing groceries, and review what Cook Mode uses from the kitchen.", icon: ChefHat, art: "meals", tone: "yellow" },
  { label: "Fam AI", title: "Helpful suggestions when you need them.", previewHeadline: "Household context, with you in control.", copy: "Ask for meals from kitchen inventory, groceries from meals, or tasks from calendar events—then review every proposed change.", icon: Bot, art: "famai", tone: "peach" },
  { label: "Tasks", title: "Lists for every kind of life admin.", previewHeadline: "See every task or focus on one custom list.", copy: "Create colour-coded lists, assign owners, add routines, and review imports from Apple, Google Tasks, or Microsoft To Do.", icon: CheckSquare, art: "tasks", tone: "pink" },
  ...
  { label: "Shopping", title: "The list and the kitchen, connected.", previewHeadline: "Shop it, store it, use it before it expires.", copy: "Add photos, scan product brands, focus shop in-store, move purchases into fridge, freezer, or pantry, and get use-soon reminders.", icon: ShoppingCart, art: "groceries", tone: "mint" },
];

const capabilityHighlights = [
  { title: "Five calendars, your visibility", copy: "Bring in Google, Apple, Outlook, school, sports, or iCal calendars and choose private or household-shared per feed.", icon: CalendarDays, tone: "lilac" },
  { title: "Recipes through Cook Mode", copy: "Open a recipe with photos and instructions, then move through each step hands-free.", icon: ChefHat, tone: "yellow" },
  { title: "Kitchen inventory", copy: "Move purchased items into the fridge, freezer, or pantry and plan meals around what is already home.", icon: Refrigerator, tone: "pink" },
  { title: "Use-soon reminders", copy: "See expiring ingredients on Today, then add replacements back to the shopping list in one tap.", icon: BellRing, tone: "mint" },
  { title: "Photos and product scans", copy: "Attach a household photo or scan a barcode to keep the product image and brand visible on the list.", icon: ShoppingCart, tone: "blue" },
  { title: "Review-first task imports", copy: "Paste or upload selected Apple, Google Tasks, or Microsoft To Do lists and approve every item before import.", icon: FileInput, tone: "peach" },
  { title: "A palette for every person", copy: "Choose a subtle colour scheme during onboarding or later in Settings, with light and dark modes tuned together.", icon: Palette, tone: "lilac" },
  { title: "Broadcasts without chat clutter", copy: "Send a household update to everyone’s Today page, where family members can react without creating another thread.", icon: MessageCircle, tone: "yellow" },
  { title: "Fam AI, review first", copy: "Connect meals, groceries, tasks, and calendars while keeping every proposed action behind your confirmation.", icon: Bot, tone: "mint" },
];
...
export default function Landing({ signedIn = false }) {
  ...
  return <MotionConfig reducedMotion="user"><div className="landing-page">
    ...
    <main>
      ...
      <motion.section className="landing-community" {...revealBlock}><div className="community-avatars" aria-hidden="true"><img src="/marketing/testimonials/maya.png" alt=""/><img src="/marketing/testimonials/jordan.png" alt=""/><img src="/marketing/testimonials/sam.png" alt=""/></div><p>One home for everyone.</p><motion.h2 {...revealHeading}>Built for every family<br/>stage.</motion.h2><span>From first appointments to school runs, teen schedules, and extended family care, FamOS keeps everyone on the same page.</span><button onClick={()=>go(signedIn?"today":"signup")}>{signedIn?"Open your family space":"Get started"}<ArrowRight/></button><motion.div className="community-facts" {...revealGroup}><motion.article variants={fadeUpSmall}><b>Private Shared Home</b><small>Roles and visibility for your people.</small></motion.article><motion.article variants={fadeUpSmall}><b>Meals & Kitchen</b><small>Recipes, Cook Mode, and inventory.</small></motion.article><motion.article variants={fadeUpSmall}><b>Connect 5 Calendars</b><small>Private or shared, with Google sync.</small></motion.article><motion.article variants={fadeUpSmall}><b>Custom Task Lists</b><small>Owners, routines, and review-first imports.</small></motion.article><motion.article variants={fadeUpSmall}><b>Review-first Fam AI</b><small>Useful proposals that wait for approval.</small></motion.article></motion.div></motion.section>
