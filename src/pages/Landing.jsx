...
const features = [
  ...
  { label: "Meals", title: "Dinner plans, minus the debate.", copy: "Plan one or two weeks, save recipes, set dietary notes, and launch a hands-free cook mode.", icon: ChefHat, art: "meals", tone: "yellow" },
  ...
  { label: "Groceries", title: "The list that remembers.", copy: "Quick-add favourites, scan barcodes, focus shop in-store, then copy or share the list to your grocery app.", icon: ShoppingCart, art: "groceries", tone: "mint" },
  { label: "Family chat", title: "Family chat, with context.", copy: "Message the household without losing the plan you were talking about. Revolutionary, we know.", icon: MessageCircle, art: "chat", tone: "blue" },
  { label: "Fam AI", title: "The next move, not another essay.", copy: "Ask for meals from groceries, groceries from meals, tasks from events, or store ideas from chores.", icon: Bot, art: "famai", tone: "peach" },
];
    ...
    <div className="pricing-shell">
      <div className="pricing-main">
        ...
        <article className="pricing-card">
          ...
          <p className="pricing-note">{billing === "annual" ? `${formatMoney(annualTotal)} billed yearly after your ${PRICING_PLAN.trial.days}-day free trial — about ${formatMoney(annualMonthlyEquivalent)}/month.` : `Billed monthly after your ${PRICING_PLAN.trial.days}-day free trial. Card required. No surprise choreography.`}</p>
...
export default function Landing({ signedIn = false }) {
  ...
  return <div className="landing-page" ref={root}>
    ...
    <main>
      ...
      <section className="landing-purpose" id="app-purpose"><div><p>Meet FamOS</p><h2>Your family’s operating system, minus the corporate energy.</h2><span>FamOS helps households coordinate shared calendars, multiple Google Calendars, meal planning, saved recipes, dietary preferences, grocery lists, barcode scanning, family chat, tasks, and AI-assisted planning in one secure home space.</span><div className="purpose-actions"><button onClick={() => go(signedIn ? "today" : "signup")}>{signedIn ? "Open FamOS" : "Start free trial"}<ArrowRight/></button>{!signedIn&&<button onClick={() => go("signin")}>Sign in</button>}</div></div><div className="purpose-grid"><article><CalendarDays/><h3>Calendars that talk</h3><p>Sync multiple Google Calendars, colour-code event types, and add places with Maps suggestions.</p></article><article><Users/><h3>Everyone gets the memo</h3><p>Invite your people, share updates, chat, and give tasks a clear owner.</p></article><article><LockKeyhole/><h3>Private by default</h3><p>Your home space belongs to your household. Disconnect access anytime.</p></article></div></section>
      ...
      <section className="landing-community"><div className="community-avatars" aria-hidden="true"><img src="/marketing/testimonials/maya.png" alt=""/><img src="/marketing/testimonials/jordan.png" alt=""/><img src="/marketing/testimonials/sam.png" alt=""/></div><p>One household. One shared rhythm.</p><h2>Built for every family<br/>with a lot to coordinate.</h2><span>From the first appointment to school runs, teen schedules, and care across generations—FamOS keeps the people who matter on the same page.</span><button onClick={()=>go(signedIn?"today":"signup")}>{signedIn?"Open your family space":"Bring your family together"}<ArrowRight/></button><div className="community-facts"><article><b>Shared Household</b><small>One private space for your people.</small></article><article><b>Meal Planning</b><small>Recipes, cook mode, and dietary needs.</small></article><article><b>Sync Multiple Calendars</b><small>Google calendars, colours, and places.</small></article><article><b>Assign Chores & Rewards</b><small>Owners now, rewards when ready.</small></article><article><b>Get Help with Fam AI</b><small>The next move, not another essay.</small></article></div></section>
