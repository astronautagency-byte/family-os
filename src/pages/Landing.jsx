...
const testimonials = [
  { quote: "We stopped asking ‘who knew about this?’ Fam OS gives everyone the same version of the week.", name: "Maya", detail: "Parent of two · Toronto", avatar: "/marketing/testimonials/maya.png" },
...
function PricingSection({ signedIn }) {
  ...
  return <section className="landing-pricing" id="pricing">
    <div className="landing-section-head"><p>Pricing</p><h2>Start with your household.<br/>Scale as your family grows.</h2><span>Try Fam OS free for 30 days. The base plan includes 3 family members, then each extra member is $3.99/month.</span></div>
...
export default function Landing({ signedIn = false }) {
  ...
  return <div className="landing-page" ref={root}>
    ...
    <main>
      ...
      <section className="landing-testimonials"><div className="landing-section-head"><p>From the kitchen table</p><h2>Made for families<br/>with a lot going on.</h2><span>Illustrative feedback showing the kind of relief Fam OS is designed to create.</span></div><div>{testimonials.map((item)=><article key={item.name}><span>{"★★★★★"}</span><blockquote>“{item.quote}”</blockquote><footer><img src={item.avatar} alt="" aria-hidden="true"/><span><b>{item.name}</b><small>{item.detail}</small></span></footer></article>)}</div><small className="testimonial-note">Sample testimonials and AI-generated portraits for design demonstration.</small></section>
      ...
      <section className="landing-final"><img src="/illustrations/famos-family-planning.png" alt="A family planning together"/><div><p>Your family, in sync.</p><h2>Families run better<br/>on Fam OS.</h2><button onClick={() => go(signedIn ? "today" : "signup")}>{signedIn ? "Open FamOS" : "Start your family space"}<ArrowRight/></button></div></section>
    </main>

    <footer className="landing-footer"><div className="landing-brand"><img src="/brand/famos-icon-transparent.png" alt=""/><strong>Fam<span>OS</span></strong></div><p>Whatever your family is. Wherever your family is.</p><div>{!signedIn&&<button onClick={() => go("signin")}>Sign in</button>}<a href="#pricing">Pricing</a><button onClick={() => go("privacy")}>Privacy</button><button onClick={() => go("terms")}>Terms</button><button onClick={() => go(signedIn ? "today" : "signup")}>{signedIn ? "Dashboard" : "Sign up"}</button></div><small>© 2026 FamOS. All rights reserved.<br/>Developed by the team at <a href="https://getastronaut.io" target="_blank" rel="noreferrer">Astronaut Digital</a> · Part of Astronaut Ventures</small></footer>
  </div>;
}
