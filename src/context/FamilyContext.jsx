...
export function FamilyProvider({ children, tabletMode = false }) {
  ...
  const setMealForSlot = async (date, slot, patch) => {
    ...
    if (remote) {
      try {
        const { data, error } = await supabase.from("meals").upsert({ household_id: household.id, meal_date: date, slot, title: patch.title || "", notes: patch.notes || "", cook_ids: patch.cookIds || [], created_by: user.id, source, thumbnail: patch.thumbnail || "", recipe_snapshot:patch.recipeSnapshot || null }, { onConflict: "household_id,meal_date,slot" }).select().single();
        if (error) throw error;
