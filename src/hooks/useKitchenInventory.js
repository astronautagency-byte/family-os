...
export default function useKitchenInventory(householdId, userId) {
  ...
  const addItem = useCallback(async (input) => {
    ...
    const local = existing
      ? { ...existing, quantity: Number(existing.quantity) + Number(input.quantity || 1), unit: input.unit || existing.unit, expiresOn: input.expiresOn || existing.expiresOn }
      : { id: makeId(), name, quantity: Number(input.quantity || 1), unit: input.unit || "", location: input.location || "fridge", expiresOn: input.expiresOn || "", sourceGroceryId: input.sourceGroceryId || null };
    persistLocal(existing ? items.map((item) => item.id === existing.id ? local : item) : [...items, local]);
    if (supabase && householdId) {
      const payload = { household_id: householdId, name, quantity: local.quantity, unit: local.unit, location: local.location, expires_on: local.expiresOn || null, source_grocery_id: local.sourceGroceryId, added_by: userId };
      const query = existing && remoteReady ? supabase.from("kitchen_inventory").update(payload).eq("id", existing.id).select().single() : supabase.from("kitchen_inventory").insert(payload).select().single();
