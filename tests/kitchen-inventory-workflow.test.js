...
test("kitchen inventory supports direct logging and an expiry lifecycle", () => {
  assert.match(groceries, /Add fresh food to Kitchen Watch/);
...
test("kitchen inventory launches meal ideas using what is at home", () => {
  ...
  assert.match(groceries, /kitchenWatchItems\.length > 0/);
...
test("kitchen inventory has one clear manual add path", () => {
  assert.match(groceries, /> Add fresh item</);
  assert.doesNotMatch(groceries, /Add your first item/);
  assert.match(groceries, /Checked perishables from Shopping also appear here for a quick review/);
});
...
test("kitchen inventory behaves like a practical food tracker", () => {
  ...
  assert.match(groceries, /\["Produce", "Deli & Prepared Foods", "Dairy & Eggs", "Meat & Seafood", "Bakery"\]/);
  assert.match(groceries, /Everything else stays on the Shopping list/);
  assert.match(groceries, /isKitchenWatchCategory\(category\)/);
